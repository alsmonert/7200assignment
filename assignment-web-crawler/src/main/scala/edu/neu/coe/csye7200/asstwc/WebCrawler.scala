...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    // 16 points.
    def getURLs(ns: Node): Seq[Try[URL]] = for (n <- ns \\ "a"  map (_ \ "@href") map (_ toString) toList) yield createURL(Some(u), n) // TO BE IMPLEMENTED
    ...
    // 9 points.
    for (g <- getURLContent(u); res <- MonadOps.asFuture( getLinks(g))) yield res// TO BE IMPLEMENTED
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
    val value = for ( u <- us2 ) yield wget(u)
    Future.sequence(MonadOps.sequence(value))
  }
